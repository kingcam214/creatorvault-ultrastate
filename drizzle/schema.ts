...
// ============================================================
// RECRUITER OS AND CONVERSION ENGINE
// ============================================================
export const recruiterCreatorProfiles = mysqlTable("recruiter_creator_profiles", {
  ...
  engagementRate: decimal("engagement_rate", { precision: 8, scale: 2 }).default("0.00").notNull(),
  ...
  scoreBreakdown: json("score_breakdown").$type<Record<string, unknown>>(),
  auditPreview: json("audit_preview").$type<Record<string, unknown>>(),
  ...
  priority: varchar("priority", { length: 40 }).default("medium").notNull(),
  ...
  platformHandleIdx: index("idx_recruiter_profiles_platform_handle").on(table.platform, table.handle),
  totalScoreIdx: index("idx_recruiter_profiles_total_score").on(table.totalScore),
  priorityIdx: index("idx_recruiter_profiles_priority").on(table.priority),
  statusIdx: index("idx_recruiter_profiles_status").on(table.status),
}));
...
export const creatorHighTicketPackages = mysqlTable("creator_high_ticket_packages", {
  ...
  description: text("description").notNull(),
  existingSystems: json("existing_systems").$type<string[]>().default([]).notNull(),
  ...
  moneyTrigger: text("money_trigger").notNull(),
  activationRoute: varchar("activation_route", { length: 255 }).notNull(),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  packageKeyIdx: index("idx_high_ticket_package_key").on(table.packageKey),
  activePriceIdx: index("idx_high_ticket_active_price").on(table.isActive, table.priceFloorCents),
}));
...
export const creatorConversionIntelligence = mysqlTable("creator_conversion_intelligence", {
  ...
  nicheClassification: text("niche_classification").notNull(),
  monetizationGaps: json("monetization_gaps").$type<string[]>().default([]).notNull(),
  ...
  signalSnapshot: json("signal_snapshot").$type<Record<string, unknown>>(),
  generatedAt: timestamp("generated_at").defaultNow().notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  profileIdIdx: index("idx_conversion_intelligence_profile_id").on(table.profileId),
...
export const creatorConversionPackets = mysqlTable("creator_conversion_packets", {
  ...
  monetizationVisuals: json("monetization_visuals").$type<Record<string, unknown>>(),
  connectedSystems: json("connected_systems").$type<string[]>().default([]).notNull(),
  packetStatus: varchar("packet_status", { length: 40 }).default("generated").notNull(),
  generatedAt: timestamp("generated_at").defaultNow().notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  profileIdIdx: index("idx_conversion_packets_profile_id").on(table.profileId),
...
export const creatorConversionAutomation = mysqlTable("creator_conversion_automation", {
  id: int("id").autoincrement().primaryKey(),
  profileId: int("profile_id").notNull().references(() => recruiterCreatorProfiles.id, { onDelete: "cascade" }),
  packetId: int("packet_id").notNull().references(() => creatorConversionPackets.id, { onDelete: "cascade" }),
  assignedRecruiter: varchar("assigned_recruiter", { length: 160 }).notNull(),
  followUpCadence: json("follow_up_cadence").$type<Record<string, unknown>>(),
  nextFollowUpAt: timestamp("next_follow_up_at"),
  lastResponseSignal: text("last_response_signal"),
  ...
  onboardingStageHistory: json("onboarding_stage_history").$type<Record<string, unknown>[]>().default([]).notNull(),
  ...
  moneyNextAction: text("money_next_action").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  profileIdIdx: index("idx_conversion_automation_profile_id").on(table.profileId),
  nextFollowUpIdx: index("idx_conversion_automation_next_follow_up").on(table.nextFollowUpAt),
  stageIdx: index("idx_conversion_automation_stage").on(table.conversionStage),
}));
...
export const creatorAcquisitionPriorities = mysqlTable("creator_acquisition_priorities", {
  ...
  nextMoneyAction: text("next_money_action").notNull(),
  scoredAt: timestamp("scored_at").defaultNow().notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  profileIdIdx: index("idx_acquisition_priorities_profile_id").on(table.profileId),
