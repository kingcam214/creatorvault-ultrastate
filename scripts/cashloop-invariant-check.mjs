...
async function verifyStripeSubscriptions(conn, activeSubscriptionRows) {
  ...
  const stripe = new Stripe(process.env.STRIPE_SECRET_KEY, { timeout: 12000, maxNetworkRetries: 0 });
  const rows = activeSubscriptionRows.filter((row) => row.stripe_subscription_id).slice(0, 50);
  const mismatches = [];
  const verified = [];
  for (const row of rows) {
    try {
      const stripeSub = await stripe.subscriptions.retrieve(row.stripe_subscription_id);
      const livePeriodEnd = stripeSub.current_period_end ? new Date(stripeSub.current_period_end * 1000).toISOString() : null;
      verified.push({
        db_subscription_id: row.id,
        db_status: row.status,
        stripe_status: stripeSub.status,
        stripe_subscription_id: row.stripe_subscription_id,
        db_current_period_end: row.current_period_end,
        stripe_current_period_end: livePeriodEnd,
      ...
      const missing = error?.code === "resource_missing" || /No such subscription/i.test(error?.message || "");
      const status = missing ? "missing_in_stripe" : "stripe_lookup_error";
      verified.push({ db_subscription_id: row.id, db_status: row.status, stripe_status: status, stripe_subscription_id: row.stripe_subscription_id, error_type: error?.type, error_code: error?.code });
      mismatches.push({ db_subscription_id: row.id, db_status: row.status, stripe_status: status, stripe_subscription_id: row.stripe_subscription_id, error_code: error?.code, error_message: error?.message });
    }
  }
