...
const file = '/home/ubuntu/creatorvault-ultrastate-clean/server/routers/telegramFunnelRouter.ts';
...
if (!src.includes('CREATOR_ACQUISITION_SEGMENTS')) {
  src = src.replace('function genTrackingCode(prefix = "tg"): string {\n  return `${prefix}${Date.now().toString(36)}${crypto.randomBytes(3).toString("hex")}`;\n}\n', 'function genTrackingCode(prefix = "tg"): string {\n  return `${prefix}${Date.now().toString(36)}${crypto.randomBytes(3).toString("hex")}`;\n}\n' + helper);
}
...
if (!src.includes('"acquisition.activateAllSegments"')) {
  src = src.replace('export const telegramFunnelRouter = router({\n', 'export const telegramFunnelRouter = router({\n' + procedure);
}

fs.writeFileSync(file, src);
console.log('Patched telegramFunnelRouter.ts with all-segment acquisition activation.');
