...
export const vaultRemixRouter = router({
  getData: protectedProcedure.query(async ({ ctx }) => ({ data: [], userId: ctx.user.id, feature: "content remixing and collaboration" })),
    // @ts-ignore
  ...
  getInsights: protectedProcedure.query(async ({ ctx }) => {
    const c = await openai.chat.completions.create({ model: "gpt-4o-mini", messages: [{ role: "user", content: `Provide insights for content remixing and collaboration feature. What are the top 3 actions to maximize results?` }], max_tokens: 300 });
