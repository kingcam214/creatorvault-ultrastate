...
import {
  ...
  launchKingcamWanAnimateControlledPerformanceBenchmark,
...
export const kingcamCloneOperatingSystemRouter = router({
  ...
  bindControlledPerformanceBenchmarkSource: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ownerOnly(ctx.user.id);
      try {
        return await bindKingcamControlledPerformanceBenchmarkSource({ ownerId: ctx.user.id, ...input });
      } catch (error) {
        throw new TRPCError({ code: "PRECONDITION_FAILED", message: error instanceof Error ? error.message : "KingCam controlled-performance source could not be verified." });
      }
    }),
  ...
  launchWanAnimateControlledPerformanceBenchmark: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ownerOnly(ctx.user.id);
      try {
        return await launchKingcamWanAnimateControlledPerformanceBenchmark({ ownerId: ctx.user.id, sceneBrief: input.sceneBrief });
      } catch (error) {
        throw new TRPCError({ code: "PRECONDITION_FAILED", message: error instanceof Error ? error.message : "KingCam controlled-performance motion benchmark could not launch." });
      }
    }),
