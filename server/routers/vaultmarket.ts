...
export const vaultmarket = router({
  getData: protectedProcedure.query(async ({ ctx }) => ({ data: [], userId: ctx.user.id, feature: "vault marketplace" })),
  ...
  getInsights: protectedProcedure.query(async ({ ctx }) => {
    const c = await openai.chat.completions.create({ model: "gpt-4o-mini", messages: [{ role: "user", content: `Provide insights for vault marketplace feature. What are the top 3 actions to maximize results?` }], max_tokens: 300 });
...
export const vaultmarketRouter = vaultmarket;
