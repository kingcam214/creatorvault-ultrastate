...
export async function launchKingcamWanAnimateControlledPerformanceBenchmark(input: { ownerId: number; sceneBrief: string }) {
  ...
  const driverUrl = "https://creatorvault.live/uploads/content-vault/e46fd473-3bfb-45da-bbb8-47f8e03e26bf/kingcam-controlled-performance-0135-0147.mov";
  if (!controlled || controlled.status !== "source_verified" || controlled.sourceMediaAssetId !== "4c5fcd40-aa8a-4553-b89e-84a468a6452a") throw new Error("KingCam controlled performance needs the verified twelve-second IMG_4392 technical Gold Standard source before one benchmark can launch.");
  const motionRequestId = randomUUID();
  await rawExec(`INSERT INTO kingcam_clone_motion_requests (id, clone_id, owner_id, source_url, source_kind, motion_reference_url, intended_lane, candidate_models_json, scene_brief, hard_credit_cap, consent_confirmed, ownership_confirmed, quality_gate_json, state, created_at, updated_at) VALUES (?, ?, ?, ?, 'verified_kingcam_motion', ?, 'governed_replicate_wan_animate_controlled_performance', ?, ?, ?, 1, 1, ?, 'planned', NOW(), NOW())`, [motionRequestId, KINGCAM_CLONE_ID, input.ownerId, KINGCAM_WAN_ANIMATE_REAL_DRIVER_IMAGE, driverUrl, json(["replicate/wan-video/wan-2.2-animate-animation"]), sceneBrief, 2, json(WAN_ANIMATE_REAL_DRIVER_QUALITY_GATES)]);
  const drafted = await createGovernedKingcamReplicateWanAnimateDraft({ creatorId: input.ownerId, requestedBy: input.ownerId, prompt: `Transfer only the real full-body controlled performance, timing, hands, feet, framing, and natural movement from the verified KingCam driving video onto the approved KingCam identity image. Keep the output silent, full body, stable, natural, and free of text, crop, spin, cuts, wardrobe drift, body distortion, plastic movement, or anatomy defects. ${sceneBrief}`, ownershipConfirmed: true, consentConfirmed: true, idempotencyKey: `kingcam-wan-animate-controlled-performance:${motionRequestId}`, requestId: motionRequestId, motionDriverUrl: driverUrl, metadata: { kingcamCloneId: KINGCAM_CLONE_ID, kingcamMotionRequestId: motionRequestId, benchmarkCaseKey: "controlled-performance", sourceMediaAssetId: controlled.sourceMediaAssetId, sourceEvidenceId: controlled.sourceEvidenceId, sourceLineage: "IMG_4392.MOV technical-only 01:35-01:47 trim", sceneBrief, qualityGate: WAN_ANIMATE_REAL_DRIVER_QUALITY_GATES } });
  const approved = await approveGovernedPolloJob({ jobId: drafted.job.id, approverId: input.ownerId, expectedFingerprint: drafted.job.fingerprint, reason: "Owner-directed KingCam Wan Animate controlled-performance benchmark. One silent seven-second output only, $2 ceiling, no automatic retry, and mandatory identity, body, hands, feet, natural-motion, framing, wardrobe, environment, and anatomy rejection gates." });
  await authorizeSingleUseGovernedPolloSubmission({ jobId: approved.id, ownerId: input.ownerId, expectedFingerprint: approved.fingerprint, hardCreditCap: 2, expiresInMinutes: 10, reason: "One-time KingCam Wan Animate controlled-performance benchmark with a locked $2 ceiling and no automatic retry." });
  const submitted = await submitGovernedPolloJob({ jobId: approved.id, workerId: `kingcam-wan-animate-controlled-performance-owner-${input.ownerId}` });
  const localState: MotionRequestState = submitted.state === "submitted" ? "submitted" : submitted.state === "failed" ? "failed" : "approved";
  await rawExec("UPDATE kingcam_clone_motion_requests SET state = ?, review_json = ?, updated_at = NOW() WHERE id = ? AND clone_id = ? AND owner_id = ?", [localState, json({ governedJobId: submitted.id, providerJobId: submitted.providerJobId, state: submitted.state, hardCreditCap: 2, providerModelPath: "replicate/wan-video/wan-2.2-animate-animation", identityImage: KINGCAM_WAN_ANIMATE_REAL_DRIVER_IMAGE, motionDriverUrl: driverUrl, qualityGate: WAN_ANIMATE_REAL_DRIVER_QUALITY_GATES }), motionRequestId, KINGCAM_CLONE_ID, input.ownerId]);
  await recordKingcamCloneMemory({ ownerId: input.ownerId, kind: "motion_proof_planned", room: "KingCam controlled-performance full-body motion", payload: { motionRequestId, governedJobId: submitted.id, providerJobId: submitted.providerJobId, state: submitted.state, hardCreditCap: 2, providerModelPath: "replicate/wan-video/wan-2.2-animate-animation", motionDriverUrl: driverUrl, qualityGate: WAN_ANIMATE_REAL_DRIVER_QUALITY_GATES } });
  return { motionRequestId, governedJob: submitted, hardCreditCap: 2, qualityGate: WAN_ANIMATE_REAL_DRIVER_QUALITY_GATES };
}
...
export async function launchKingcamKlingOmniControlledPerformanceBenchmark(input: { ownerId: number; sceneBrief: string }) {
  ...
  await rawExec(`INSERT INTO kingcam_clone_motion_requests (id, clone_id, owner_id, source_url, source_kind, motion_reference_url, intended_lane, candidate_models_json, scene_brief, hard_credit_cap, consent_confirmed, ownership_confirmed, quality_gate_json, state, created_at, updated_at) VALUES (?, ?, ?, ?, 'verified_kingcam_motion', ?, 'governed_pollo_kling_omni_controlled_performance_benchmark', ?, ?, ?, 1, 1, ?, 'planned', NOW(), NOW())`, [motionRequestId, KINGCAM_CLONE_ID, input.ownerId, KINGCAM_FULL_BODY_IMAGE, motionDriverUrl, json(["pollo/kling-ai/kling-v3-omni-ref2video"]), sceneBrief, 11.13, json(qualityGate)]);
