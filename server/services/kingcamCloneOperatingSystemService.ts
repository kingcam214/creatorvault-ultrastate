...
export async function launchKingcamWanAnimateControlledPerformanceBenchmark(input: { ownerId: number; sceneBrief: string }) {
  ...
  const driverUrl = "https://creatorvault.live/uploads/content-vault/36bd1982-8de5-4581-9896-70dcb21be904/IMG_4392.MOV";
  if (!controlled || controlled.status !== "source_verified" || controlled.sourceMediaAssetId !== "d9ec0e36-528b-4f96-8004-77b0af3e4f68") throw new Error("KingCam controlled performance needs the verified IMG_4392 Gold Standard source before one benchmark can launch.");
  const motionRequestId = randomUUID();
