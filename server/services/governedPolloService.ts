...
const REPLICATE_WAN_ANIMATE_CONTROLLED_PERFORMANCE_DRIVER_URL = "https://creatorvault.live/uploads/content-vault/e46fd473-3bfb-45da-bbb8-47f8e03e26bf/kingcam-controlled-performance-0135-0147.mov";
...
const KINGCAM_ACTION_IMITATION_V2_MODE = "kingcam_action_imitation_v2_real_driver_motion";
...
const KINGCAM_KLING_OMNI_SPOKEN_MOTION_DRIVER_URL = "https://creatorvault.live/uploads/content-vault/c51e95c5-eea0-489f-90b6-0d893c12ecb8/kingcam-controlled-performance-sdr-v2.mp4";
const KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODEL_PATH = "pollo/happyhorse/happyhorse-1-0-ref";
const KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODE = "kingcam_happyhorse_all_reference_full_performer";
const KINGCAM_HAPPYHORSE_ALL_REFERENCE_HARD_CREDIT_CAP = 75;
const KINGCAM_HAPPYHORSE_ALL_REFERENCE_DURATION_SECONDS = 15;
const KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL = KINGCAM_KLING_OMNI_SPOKEN_MOTION_IMAGE_URL;
const KINGCAM_HAPPYHORSE_BODY_MOTION_VIDEO_URL = KINGCAM_KLING_OMNI_SPOKEN_MOTION_DRIVER_URL;
const KINGCAM_HAPPYHORSE_FACE_PERFORMANCE_VIDEO_URL = "https://creatorvault.live/uploads/content-vault/095edefc-59d3-4951-a871-630303b01b60/43df2204-c955-4b9e-968a-3bde65e30fbe.mp4";
const KINGCAM_HAPPYHORSE_REAL_VOICE_AUDIO_URL = KINGCAM_KLING_OMNI_SPOKEN_MOTION_AUDIO_URL;
...
function isKingcamHappyHorseAllReferenceJob(job: Pick<GovernedPolloJob, "provider" | "providerModelPath" | "mode" | "metadata">): boolean {
  return job.provider === "pollo"
    && job.providerModelPath === KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODEL_PATH
    && job.mode === KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODE
    && job.metadata.kingcamHappyHorseAllReferenceProof === true
    && job.metadata.ownerDirectedPilot === true
    && job.metadata.candidateLimit === 1
    && job.metadata.noAutomaticRetry === true
    && job.metadata.sourcePreservationRequired === true
    && job.metadata.genericVoiceFallbackForbidden === true
    && job.metadata.identityReferenceUrl === KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL
    && job.metadata.motionDriverUrl === KINGCAM_HAPPYHORSE_BODY_MOTION_VIDEO_URL
    && job.metadata.facePerformanceVideoUrl === KINGCAM_HAPPYHORSE_FACE_PERFORMANCE_VIDEO_URL
    && job.metadata.audioUrl === KINGCAM_HAPPYHORSE_REAL_VOICE_AUDIO_URL
    && job.metadata.hardCreditCap === KINGCAM_HAPPYHORSE_ALL_REFERENCE_HARD_CREDIT_CAP;
}
...
function isKingcamActionImitationV2Job(job: Pick<GovernedPolloJob, "provider" | "providerModelPath" | "mode" | "sourceUrl" | "metadata">): boolean {
  return job.provider === "pollo"
    && job.providerModelPath === KINGCAM_ACTION_IMITATION_V2_MODEL_PATH
    && job.mode === KINGCAM_ACTION_IMITATION_V2_MODE
    && job.sourceUrl === KINGCAM_ACTION_IMITATION_V2_DRIVER_URL
    && job.metadata.kingcamActionImitationV2RealDriverProof === true
    && job.metadata.cloneOnly === true
    && job.metadata.ownerDirectedPilot === true
    && job.metadata.candidateLimit === 1
    && job.metadata.noAutomaticRetry === true
    && job.metadata.sourcePreservationRequired === true
    && job.metadata.hardCreditCap === KINGCAM_ACTION_IMITATION_V2_HARD_CREDIT_CAP
    && job.metadata.identityImageUrl === KINGCAM_ACTION_IMITATION_V2_IDENTITY_IMAGE_URL
    && job.metadata.motionDriverUrl === KINGCAM_ACTION_IMITATION_V2_DRIVER_URL
    && job.metadata.bodyCinemaExcluded === true;
}

function isKingcamKlingOmniRealGaitJob(job: Pick<GovernedPolloJob, "provider" | "providerModelPath" | "mode" | "sourceUrl" | "metadata">): boolean {
...
export async function auditPolloKingcamActionImitationV2Candidate(): Promise<{
  ...
  const estimateInput = {
    image: REPLICATE_WAN_ANIMATE_IDENTITY_IMAGE_URL,
    video: KINGCAM_GOENHANCE_REAL_DRIVER_URL,
  };
  let response: Response;
  try {
    response = await fetch("https://pollo.ai/api/platform/v1/generation/pollo-ai/action-imitation-v2/motion/estimate", {
      method: "POST",
      headers: { "x-api-key": apiKey, "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ input: estimateInput }),
    });
  } catch (error) {
    return { providerModelKey: "pollo-ai/action-imitation-v2", apiPath: KINGCAM_ACTION_IMITATION_V2_API_PATH, configAvailable: false, quotedCredits: null, quotedCostUsd: null, eligibleForDraft: false, reason: `The official Action Imitation V2 estimate could not be read: ${safeErrorMessage(error)}`, providerRecord: null };
  }
  const payload = await parseProviderJson(response);
  if (!response.ok) {
    return { providerModelKey: "pollo-ai/action-imitation-v2", apiPath: KINGCAM_ACTION_IMITATION_V2_API_PATH, configAvailable: false, quotedCredits: null, quotedCostUsd: null, eligibleForDraft: false, reason: `Pollo returned ${response.status} from the official Action Imitation V2 estimate: ${safeErrorMessage(payload.responseText ?? payload.message ?? "unknown error")}`, providerRecord: null };
  }
  const record = isProviderRecord(payload.data) ? payload.data : isProviderRecord(payload) ? payload : null;
  if (!record) {
    return { providerModelKey: "pollo-ai/action-imitation-v2", apiPath: KINGCAM_ACTION_IMITATION_V2_API_PATH, configAvailable: false, quotedCredits: null, quotedCostUsd: null, eligibleForDraft: false, reason: "Pollo accepted the no-charge Action Imitation V2 estimate request without a readable cost record. No draft or provider task was created.", providerRecord: null };
  }
  ...
  return {
    ...
    reason: quotedCredits !== null && quotedCredits > 0
      ? "Pollo returned an official Action Imitation V2 estimate for the locked real KingCam image and motion-video inputs. This is evidence only; a separate governed contract and owner-directed decision are required before any draft can exist."
...
export async function auditPolloKingcamKlingOmniRealGaitCandidate(): Promise<{
  providerModelKey: string;
  apiPath: string;
  configAvailable: boolean;
  quotedCredits: number | null;
  ...
  const apiKey = String(process.env.POLLO_API_KEY || "").trim();
  if (!apiKey) return {
    providerModelKey: "kling-ai/kling-v3-omni-ref2video",
    apiPath: KINGCAM_KLING_OMNI_REAL_GAIT_ESTIMATE_API_PATH,
    configAvailable: false,
    quotedCredits: null,
    quotedCostUsd: null,
    eligibleForDraft: false,
    reason: "Pollo credential is unavailable; no estimate or provider task was requested.",
    providerRecord: null,
  };
  let response: Response;
  try {
    response = await fetch(`https://pollo.ai/api/platform${KINGCAM_KLING_OMNI_REAL_GAIT_ESTIMATE_API_PATH}`, {
      ...
      body: JSON.stringify({
        input: {
          prompt: "KingCam clone-only real-gait motion transfer using the approved KingCam identity image and the locked seven-second real gait video. Preserve full body, burgundy suit with gold embroidery, crown, jewelry, black shoes, right-hand cigar, dark lounge geometry, grounded gait, and natural hands and feet. No crop, no spin, no audio, no substitution, no environment drift, and no added text.",
          refs: [
            { type: "image", name: "KingCam identity", image: KINGCAM_KLING_OMNI_SPOKEN_MOTION_IMAGE_URL, order: 1 },
            { type: "video", name: "KingCam real gait", video: REPLICATE_WAN_ANIMATE_DRIVER_URL, order: 2 },
          ],
          duration: 7,
          aspectRatio: "16:9",
          resolution: "720p",
    ...
    return {
      ...
      quotedCostUsd: null,
      eligibleForDraft: false,
      reason: `The official Kling 3 Omni real-gait estimate could not be read: ${safeErrorMessage(error)}`,
      providerRecord: null,
    };
  }
  const payload = await parseProviderJson(response);
  if (!response.ok) return {
    ...
    quotedCostUsd: null,
    eligibleForDraft: false,
    reason: `Pollo returned ${response.status} from the official Kling 3 Omni real-gait estimate: ${safeErrorMessage(payload.responseText ?? payload.message ?? "unknown error")}`,
    providerRecord: null,
  };
  const record = isProviderRecord(payload.data) ? payload.data : isProviderRecord(payload) ? payload : null;
  if (!record) return {
    ...
    quotedCostUsd: null,
    eligibleForDraft: false,
    reason: "Pollo accepted the no-charge Kling 3 Omni real-gait estimate without a readable cost record. No draft or provider task was created.",
    providerRecord: null,
  };
  const quotedCredits = providerNumber(record, ["cost", "singleCost", "totalCost", "credit", "credits", "amount", "price", "discountCost"]);
  const quotedCostUsd = providerNumber(record, ["costUsd", "singleCostUsd", "totalCostUsd", "usd", "amountUsd", "priceUsd", "priceUSD", "discountCostUsd"]);
  return {
    ...
    eligibleForDraft: false,
    reason: quotedCredits !== null && quotedCredits > 0
      ? "Pollo returned an official Kling 3 Omni mixed-reference estimate for the approved KingCam identity image and locked real gait video. This is no-charge evidence only; a separately governed one-output decision is still required."
...
function buildKingcamKlingOmniRealGaitInput(job: Pick<GovernedPolloJob, "sourceUrl" | "prompt" | "resolution" | "durationSeconds" | "aspectRatio" | "metadata">): Record<string, unknown> {
  if (job.sourceUrl !== REPLICATE_WAN_ANIMATE_DRIVER_URL || job.resolution !== "720p" || job.durationSeconds !== 7 || job.aspectRatio !== "16:9" || job.metadata.identityImageUrl !== KINGCAM_KLING_OMNI_SPOKEN_MOTION_IMAGE_URL || job.metadata.audioReferenceExcluded !== true) throw new Error("KingCam Kling 3 Omni real-gait proof requires the locked image-plus-real-video silent contract.");
...
function buildKingcamHappyHorseAllReferenceInput(job: Pick<GovernedPolloJob, "sourceUrl" | "prompt" | "resolution" | "durationSeconds" | "aspectRatio" | "metadata">): Record<string, unknown> {
  if (!isKingcamHappyHorseAllReferenceJob({ provider: "pollo", providerModelPath: KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODEL_PATH, mode: KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODE, metadata: job.metadata })) throw new Error("KingCam Happy Horse candidate requires the locked all-reference contract.");
  if (job.sourceUrl !== KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL || job.resolution !== "1080p" || job.durationSeconds !== KINGCAM_HAPPYHORSE_ALL_REFERENCE_DURATION_SECONDS || job.aspectRatio !== "9:16") throw new Error("KingCam Happy Horse candidate must remain one 15-second vertical 1080p output.");
  return {
    prompt: job.prompt,
    duration: KINGCAM_HAPPYHORSE_ALL_REFERENCE_DURATION_SECONDS,
    aspectRatio: "9:16",
    resolution: "1080P",
    videoNum: 1,
    refs: [
      { type: "image", name: "KingCam crown-and-suit visual identity", image: KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL, order: 1 },
      { type: "video", name: "KingCam verified SDR full-body movement", video: KINGCAM_HAPPYHORSE_BODY_MOTION_VIDEO_URL, order: 2 },
      { type: "video", name: "KingCam real face and mouth performance", video: KINGCAM_HAPPYHORSE_FACE_PERFORMANCE_VIDEO_URL, order: 3 },
      { type: "audio", name: "KingCam verified real voice", audio: KINGCAM_HAPPYHORSE_REAL_VOICE_AUDIO_URL, order: 4 },
    ],
  };
}

export async function createManualCappedKingcamHappyHorseAllReferenceDraft(input: {
  creatorId: number;
  requestedBy: number;
  prompt: string;
  manualCreditCap: 75;
  ...
  const metadata = {
    ...(input.metadata || {}),
    kingcamHappyHorseAllReferenceProof: true,
    ownerDirectedPilot: true,
    candidateLimit: 1,
    noAutomaticRetry: true,
    sourcePreservationRequired: true,
    genericVoiceFallbackForbidden: true,
    manualCreditCap: KINGCAM_HAPPYHORSE_ALL_REFERENCE_HARD_CREDIT_CAP,
    hardCreditCap: KINGCAM_HAPPYHORSE_ALL_REFERENCE_HARD_CREDIT_CAP,
    providerQuoteUnavailable: true,
    providerPriceResolution: "manual_owner_cap_with_pre_post_balance_evidence",
    identityReferenceUrl: KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL,
    motionDriverUrl: KINGCAM_HAPPYHORSE_BODY_MOTION_VIDEO_URL,
    facePerformanceVideoUrl: KINGCAM_HAPPYHORSE_FACE_PERFORMANCE_VIDEO_URL,
    audioUrl: KINGCAM_HAPPYHORSE_REAL_VOICE_AUDIO_URL,
    providerContract: "pollo_happyhorse_image_plus_body_video_plus_face_video_plus_real_audio",
  };
  buildKingcamHappyHorseAllReferenceInput({ sourceUrl: KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL, prompt: input.prompt, resolution: "1080p", durationSeconds: KINGCAM_HAPPYHORSE_ALL_REFERENCE_DURATION_SECONDS, aspectRatio: "9:16", metadata });
  return createGovernedPolloDraft({
    creatorId: input.creatorId, requestedBy: input.requestedBy, sourceUrl: KINGCAM_HAPPYHORSE_IDENTITY_IMAGE_URL,
    sourceChecksum: null, prompt: input.prompt, provider: "pollo", providerModelPath: KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODEL_PATH,
    resolution: "1080p", durationSeconds: KINGCAM_HAPPYHORSE_ALL_REFERENCE_DURATION_SECONDS, aspectRatio: "9:16",
    mode: KINGCAM_HAPPYHORSE_ALL_REFERENCE_MODE, outputCount: 1, estimatedCostCredits: KINGCAM_HAPPYHORSE_ALL_REFERENCE_HARD_CREDIT_CAP,
    costEvidenceReference: "Owner-approved cap is required before any Happy Horse all-reference KingCam test. Provider price discovery is unavailable; pre/post balance evidence is mandatory.",
    ownershipConfirmed: input.ownershipConfirmed, consentConfirmed: input.consentConfirmed,
    idempotencyKey: input.idempotencyKey, requestId: input.requestId, metadata,
  });
}
...
export async function createGovernedKingcamKlingOmniRealGaitDraft(input: { creatorId: number; requestedBy: number; ownershipConfirmed: boolean; consentConfirmed: boolean; idempotencyKey?: string | null; requestId?: string | null; metadata?: Record<string, unknown> }): Promise<{ job: GovernedPolloJob; reused: boolean }> {
...
export async function createGovernedKingcamKlingOmniControlledPerformanceDraft(input: { creatorId: number; requestedBy: number; ownershipConfirmed: boolean; consentConfirmed: boolean; idempotencyKey?: string | null; requestId?: string | null; metadata?: Record<string, unknown> }): Promise<{ job: GovernedPolloJob; reused: boolean }> {
  ...
  if (!quote.configAvailable || quote.quotedCredits !== KINGCAM_KLING_OMNI_CONTROLLED_PERFORMANCE_HARD_CREDIT_CAP) throw new Error("Kling 3 Omni controlled-performance benchmark requires the locked live 11.13-credit source-specific estimate.");
...
export async function createGovernedKingcamKlingOmniArmsHandsDraft(input: { creatorId: number; requestedBy: number; ownershipConfirmed: boolean; consentConfirmed: boolean; idempotencyKey?: string | null; requestId?: string | null; metadata?: Record<string, unknown> }): Promise<{ job: GovernedPolloJob; reused: boolean }> {
  const quote = await auditPolloKingcamKlingOmniArmsHandsCandidate();
  if (!quote.configAvailable || quote.quotedCredits !== 11.13 || quote.quotedCostUsd !== 0.667) throw new Error("Kling 3 Omni arms-and-hands benchmark requires the locked live 11.13-credit / $0.667 source-specific estimate.");
  return createGovernedPolloDraft({ creatorId: input.creatorId, requestedBy: input.requestedBy, provider: "pollo", sourceUrl: KINGCAM_KLING_OMNI_ARMS_HANDS_SOURCE_URL, sourceChecksum: null, prompt: "KingCam clone-only arms-and-hands benchmark. Preserve the real face, black shirt and sweatpants, sunglasses, gold chain, phone in the right hand, shopping bags in the left hand, real gesture timing, hands, fingers, anatomy, mirror environment, and stable camera. No audio, no walk claim, no crown, no cigar, no crop, no added person, no text, no wardrobe change, no cut, and no invented environment.", providerModelPath: KINGCAM_KLING_OMNI_ARMS_HANDS_MODEL_PATH, resolution: "720p", durationSeconds: 7, aspectRatio: "16:9", mode: KINGCAM_KLING_OMNI_ARMS_HANDS_MODE, outputCount: 1, estimatedCostCredits: quote.quotedCredits, costEvidenceReference: "Provider-verified Kling 3 Omni arms-and-hands estimate: 11.13 Pollo credits / $0.667. One silent output only, exact hard cap, and no automatic retry.", ownershipConfirmed: input.ownershipConfirmed, consentConfirmed: input.consentConfirmed, idempotencyKey: input.idempotencyKey, requestId: input.requestId, metadata: { ...(input.metadata || {}), kingcamKlingOmniArmsHandsProof: true, cloneOnly: true, ownerDirectedPilot: true, candidateLimit: 1, noAutomaticRetry: true, sourcePreservationRequired: true, providerCostCurrency: "Pollo credits", providerQuote: { credits: quote.quotedCredits, costUsd: quote.quotedCostUsd, source: "official_kling_omni_arms_hands_estimate" }, providerQuotedCredits: quote.quotedCredits, providerQuotedCostUsd: quote.quotedCostUsd, hardCreditCap: 11.13, identityImageUrl: KINGCAM_KLING_OMNI_SPOKEN_MOTION_IMAGE_URL, motionDriverUrl: KINGCAM_KLING_OMNI_ARMS_HANDS_SOURCE_URL, audioReferenceExcluded: true, providerContract: "pollo_kling_omni_image_plus_real_arms_hands_video_no_audio", bodyCinemaExcluded: true } });
}
...
export async function createGovernedKingcamActionImitationV2Draft(input: {
  ...
  const quote = await auditPolloKingcamActionImitationV2Candidate();
  if (!quote.configAvailable || quote.quotedCredits !== KINGCAM_ACTION_IMITATION_V2_HARD_CREDIT_CAP || quote.quotedCostUsd !== KINGCAM_ACTION_IMITATION_V2_QUOTED_COST_USD) {
    throw new Error("Action Imitation V2 must return the locked live 35-credit / $2.10 estimate for the exact KingCam image and gait-driver inputs before a governed draft can exist.");
  }
  return createGovernedPolloDraft({
    creatorId: input.creatorId,
    requestedBy: input.requestedBy,
    provider: "pollo",
    sourceUrl: KINGCAM_ACTION_IMITATION_V2_DRIVER_URL,
    sourceChecksum: null,
    prompt: "KingCam clone-only Action Imitation V2 real-driver full-body motion transfer. Locked approved identity image plus real seven-second gait driver; one output only; never Body Cinema.",
    providerModelPath: KINGCAM_ACTION_IMITATION_V2_MODEL_PATH,
    resolution: "720p",
    durationSeconds: 7,
    aspectRatio: "16:9",
    mode: KINGCAM_ACTION_IMITATION_V2_MODE,
    outputCount: 1,
    estimatedCostCredits: quote.quotedCredits,
    costEvidenceReference: "Provider-verified Action Imitation V2 estimate for the locked KingCam identity image and real gait source: 35 Pollo credits / $2.10. One output only; the provider estimate is the hard ceiling and no automatic retry is allowed.",
    ownershipConfirmed: input.ownershipConfirmed,
    consentConfirmed: input.consentConfirmed,
    idempotencyKey: input.idempotencyKey,
    requestId: input.requestId,
    metadata: {
      ...(input.metadata || {}),
      kingcamActionImitationV2RealDriverProof: true,
      cloneOnly: true,
      providerCostCurrency: "Pollo credits",
      providerQuote: { credits: quote.quotedCredits, costUsd: quote.quotedCostUsd, source: "official_action_imitation_v2_estimate" },
      ...
      noAutomaticRetry: true,
      sourcePreservationRequired: true,
      identityImageUrl: KINGCAM_ACTION_IMITATION_V2_IDENTITY_IMAGE_URL,
      motionDriverUrl: KINGCAM_ACTION_IMITATION_V2_DRIVER_URL,
      providerContract: "pollo_action_imitation_v2_image_plus_real_video_motion_transfer",
      bodyCinemaExcluded: true,
    },
  });
}
...
async function submitGovernedKingcamActionImitationV2Job(leased: GovernedPolloJob, workerId: string): Promise<GovernedPolloJob> {
  ...
  const identityImageUrl = typeof leased.metadata.identityImageUrl === "string" ? leased.metadata.identityImageUrl : "";
  const motionDriverUrl = typeof leased.metadata.motionDriverUrl === "string" ? leased.metadata.motionDriverUrl : "";
  if (leased.sourceUrl !== KINGCAM_ACTION_IMITATION_V2_DRIVER_URL || identityImageUrl !== KINGCAM_ACTION_IMITATION_V2_IDENTITY_IMAGE_URL || motionDriverUrl !== KINGCAM_ACTION_IMITATION_V2_DRIVER_URL) {
    return failGovernedPolloJob({ jobId: leased.id, code: "action_imitation_input_contract_mismatch", error: new Error("Action Imitation V2 requires the locked approved KingCam identity image and real gait driver."), releaseBudget: true });
  }
  const balanceBeforeCredits = await readPolloAvailableCredits(apiKey);
  ...
  let response: Response;
  try {
    response = await fetch(`https://pollo.ai/api/platform${KINGCAM_ACTION_IMITATION_V2_API_PATH}`, {
      method: "POST",
      headers: { "x-api-key": apiKey, "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ input: { image: KINGCAM_ACTION_IMITATION_V2_IDENTITY_IMAGE_URL, video: KINGCAM_ACTION_IMITATION_V2_DRIVER_URL } }),
    });
  } catch (error) {
    return markGovernedPolloSubmissionUnknown({ jobId: leased.id, workerId, error });
  }
  ...
  if (!response.ok) {
    if (response.status >= 500 || response.status === 408 || response.status === 429) {
      return markGovernedPolloSubmissionUnknown({ jobId: leased.id, workerId, error: new Error(`Pollo Action Imitation V2 submission returned ${response.status}: ${safeErrorMessage(providerResponse.responseText ?? providerResponse.message ?? "unknown error")}`) });
    }
    return failGovernedPolloJob({ jobId: leased.id, code: `action_imitation_http_${response.status}`, error: new Error(`Pollo Action Imitation V2 submission returned ${response.status}: ${safeErrorMessage(providerResponse.responseText ?? providerResponse.message ?? "unknown error")}`), releaseBudget: true });
  }
  const providerJobId = providerResponse?.data?.taskId || providerResponse?.taskId || providerResponse?.id || providerResponse?.data?.id;
  if (!providerJobId) return markGovernedPolloSubmissionUnknown({ jobId: leased.id, workerId, error: new Error("Pollo Action Imitation V2 accepted the request without a task ID") });
  return markGovernedPolloSubmitted({ jobId: leased.id, workerId, providerJobId: String(providerJobId), providerResponse });
}
...
export async function submitGovernedPolloJob(params: { jobId: number; workerId: string }): Promise<GovernedPolloJob> {
  ...
  const requestBody = isSourceVideoReferenceJob(leased)
    ...
    : isKingcamWanSpokenMotionJob(leased)
      ...
      : isKingcamHappyHorseAllReferenceJob(leased)
        ? buildKingcamHappyHorseAllReferenceInput(leased)
