...
const DEFAULT_MODELS: Array<Omit<CreationModelRegistryEntry, "createdAt" | "updatedAt">> = [
  ...
  {
    modelKey: "topaz/proteus-precision-video",
    provider: "topaz",
    ...
    supportsPoseControl: false,
    supportsAudio: true,
    knownWeaknesses: ["A precision finish is not a creative Body Cinema treatment.", "No accepted CreatorVault Topaz benchmark exists yet.", "Only the non-generative Proteus model is permitted by the protected-source contract."],
    verifiedUseCases: [],
    benchmarkEvidenceVersion: null,
    benchmarkState: "unbenchmarked",
    metadata: {
      ...
      governedOnly: true,
      sourcePreservingOnly: true,
      generativeModesExcluded: true,
      canonicalService: "server/services/topazPrecisionVideoService.ts",
    },
  },
  {
  ...
  {
    modelKey: "wan/wan2.1-vace-14b-masked-video-edit",
    provider: "creatorvault_vace",
    model: "Wan2.1 VACE 14B",
    modelVersion: "14b-masked-video-edit",
    executionLane: "controlled",
    commercialEligibility: "verified",
    licenseName: "Apache-2.0",
    licenseReference: "https://huggingface.co/Wan-AI/Wan2.1-VACE-14B",
    activationState: "blocked",
    inputModes: ["source_video", "reference_image", "reference_video"],
    outputModes: ["video"],
    ...
    supportsAudio: false,
    knownWeaknesses: [
      "No CreatorVault VACE worker is connected to an approved GPU environment.",
      "No CreatorVault accepted VACE benchmark exists.",
      "The source map currently has no verified temporal subject mask, depth control, or optical-flow controls, so VACE submission remains blocked.",
      "VACE is limited to a five-second masked source segment until watchable evidence proves longer continuity.",
    ],
    verifiedUseCases: [],
    benchmarkEvidenceVersion: null,
    benchmarkState: "unbenchmarked",
    metadata: {
      officialRepository: "https://github.com/ali-vilab/VACE",
      workerProtocol: "creatorvault-vace-worker/v1",
      governedOnly: true,
      sourcePreservationRequired: true,
      requiredControls: ["temporal_subject_mask", "depth_control", "optical_flow", "reference_frames"],
