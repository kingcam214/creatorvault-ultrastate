...
#   full   = SCP ALL frontend assets + server bundle to VPS
set -e
...
if [ "$MODE" = "server" ]; then
  ...
  echo "[2/4] SCP'ing ALL frontend assets to VPS (full sync)..."
  # Sync entire assets directory to keep chunk hashes in sync with index.html
  $SCP -r dist/public/assets/ $VPS:$APP_DIR/dist/public/
