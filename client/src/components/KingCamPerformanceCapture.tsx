...
export function KingCamPerformanceCapture() {
  ...
  const reviewRoleEvidence = trpc.kingcamCloneOperatingSystem.reviewPerformanceRoleEvidence.useMutation({
    onSuccess: (result) => {
      setNotice(`${result.performanceRoleLabel} is now a reviewed KingCam evidence layer. It strengthens that part of the clone without pretending it is the final motion driver.`);
    },
    onError: (error) => setNotice(error.message),
  });
