...
function isEligibleSocialSource(asset: any) {
...
export default function SocialHub() {
  ...
  const readyMedia = (Array.isArray(media.data) ? media.data : []).filter(isEligibleSocialSource);
        ...
        <section className="mt-8 grid gap-7 xl:grid-cols-[minmax(0,1fr)_22rem]">
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/60 to-transparent px-4 pb-4 pt-20">
                        ...
                        <div className="mt-4 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <ActionButton icon={MessageCircle} label={post.comments} onClick={() => toast({ title: "Reply from the moment", description: "Your CreatorVault community will be ready for your reply here." })} />
          ...
          <aside className="space-y-4">
            ...
            <section className="rounded-[1.5rem] border border-white/10 bg-gradient-to-br from-amber-500/10 to-transparent p-5">
              <div className="flex items-center justify-between"><div><div className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-200/60">Your next moves</div><h3 className="mt-1 text-lg font-black">Where your attention is needed</h3></div><Bell className="h-5 w-5 text-amber-300" /></div>
              <div className="mt-4 space-y-3">
                {notifications.data?.length ? notifications.data.slice(0, 4).map((note: any) => <div key={note.id} className="border-l-2 border-amber-300/70 pl-3 text-xs text-zinc-300">{note.message || note.event_type}</div>) : <p className="text-xs leading-relaxed text-zinc-500">New follows, comments, replies, reactions, purchases, and messages will show up here when they happen.</p>}
              </div>
            </section>
          </aside>
        </section>
