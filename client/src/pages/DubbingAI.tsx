...
export default function DubbingAI() {
  ...
  const activeSourceDuration = Number(activeSource?.duration || 0) > 0 ? Number(activeSource?.duration) : previewDurationSec;
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#060609] pb-20 pt-20 text-white">
      ...
      <MediaPicker open={pickerOpen} onClose={() => setPickerOpen(false)} mode="single" title="Choose Your Video Source" subtitle="Only saved CreatorVault videos that can be opened and used are offered here." confirmLabel="Use This Video" assetEligibility={isEligibleDubbingSource} onConfirm={(assets) => { const source = assets.find(isEligibleDubbingSource); if (!source) return; setPreviewDurationSec(null); setSelectedAsset(source); setPickerOpen(false); }} />
    </main>
  );
}
