  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#060609] pb-20 pt-20 text-white">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-5 py-9 sm:px-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(340px,.95fr)] lg:px-12">
        ...
        <aside className="rounded-3xl border border-cyan-200/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.12),transparent_48%),#101116] p-6 sm:p-8"><div className="flex items-start gap-3"><span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-cyan-200/10 text-cyan-100"><ShieldCheck className="h-5 w-5" /></span><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-200">Truth before output</p><h2 className="mt-1 text-2xl font-black tracking-[-0.045em] text-white">No fake dubbed track.</h2></div></div><p className="mt-6 text-sm leading-relaxed text-zinc-300">The old script-only path could turn words into a translated paragraph but could not analyze the selected video, make a governed voice track, preserve timing, or give you anything to play. It is held instead of being passed off as dubbing.</p><div className="mt-6 rounded-2xl border border-white/10 bg-black/25 p-5"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500">What earns this room back</p><p className="mt-3 text-sm leading-relaxed text-zinc-200">One owned source video → verified spoken-word analysis → governed voice track → timing and identity review → a playable dubbed result in your Media Vault.</p></div><p className="mt-6 text-xs leading-relaxed text-zinc-500">Your selected source remains available. No provider, voice model, or translation call is started here.</p></aside>
      </section>
