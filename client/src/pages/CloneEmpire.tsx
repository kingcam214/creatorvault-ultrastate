...
export default function CloneEmpire() {
  ...
  const cloneMedia = useMemo(() => {
    ...
    return vault.map((asset) => ({ id: asset.id, title: asset.originalName || asset.fileName, url: asset.publicUrl!, poster: videoPoster(asset), createdAt: asset.createdAt || null })).slice(0, 9);
  }, [media]);
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050506] pb-20 pt-20 text-white">
      ...
      <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[10px] font-black uppercase tracking-[0.17em] text-zinc-500">KingCam on camera</p><h2 className="mt-2 text-3xl font-black tracking-[-0.055em] text-white">Watch the real KingCam motion.</h2></div><p className="max-w-sm text-sm leading-relaxed text-zinc-400">Only real, playable KingCam video lives here. Half-finished attempts stay out of your library.</p></div>
        {mediaQuery.isLoading ? <div className="mt-7 grid grid-cols-2 gap-4 md:grid-cols-3">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="aspect-[9/14] animate-pulse rounded-3xl bg-white/[0.05]" />)}</div> : cloneMedia.length ? <div className="mt-7 grid grid-cols-2 gap-4 md:grid-cols-3">{cloneMedia.map((item) => <article key={item.id} className="group overflow-hidden rounded-3xl border border-white/10 bg-[#0d0d12] transition hover:-translate-y-0.5 hover:border-amber-100/40"><div className="relative aspect-[9/14] bg-black"><video src={item.url} poster={item.poster || undefined} controls autoPlay loop muted playsInline preload="auto" className="h-full w-full object-cover" /><div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/80 to-transparent" /><span className="pointer-events-none absolute left-3 top-3 inline-flex items-center gap-1 rounded-full border border-white/20 bg-black/55 px-2 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-white backdrop-blur"><Play className="h-3 w-3 fill-current" /> Play</span></div><div className="p-4"><p className="truncate text-sm font-black text-white">{item.title}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.12em] text-zinc-500">{formatDate(item.createdAt)}</p></div></article>)}</div> : <div className="mt-7 rounded-3xl border border-dashed border-white/15 bg-white/[0.02] p-8 sm:p-10"><Film className="h-9 w-9 text-zinc-600" /><h2 className="mt-5 text-2xl font-black text-white">No finished KingCam performance is ready to watch yet.</h2><p className="mt-3 max-w-xl text-sm leading-relaxed text-zinc-400">CreatorVault keeps this library empty instead of filling it with promises, half-finished clips, or substitutes. Your real moving KingCam footage remains above.</p></div>}
      </section>
    </main>
  );
}
