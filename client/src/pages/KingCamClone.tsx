...
export default function KingCamClone() {
  ...
  const launchProof = trpc.kingcamCloneOperatingSystem.launchFullBodyProof.useMutation({
    onSuccess: (result) => { setNotice(`KingCam's first full-body proof is now in the governed Pollo lane. Job ${result.governedJob.id} has one output only and a fixed ${result.quote.quotedCredits}-credit ceiling.`); command.refetch(); },
    onError: (error) => setNotice(error.message),
  });
