...
const fullBodyBrief = [
  "A full-body KingCam spoken-motion proof using the approved CreatorVault KingCam identity image and his verified direct voice.",
  "KingCam remains fully visible from crown to shoes in the approved burgundy royal look, visibly delivers his own words, takes one natural measured step toward the camera, shifts his weight naturally, and holds a confident grounded stance with both feet fully visible.",
  ...
  "His mouth must match the real KingCam audio. No overlay voice, no recycled footage, no talking-head substitution, no extra people, no body drift, no warped hands or cigar, no new outfit, no text, no watermark, no plastic skin, and no artificial-looking movement.",
].join(" ");
...
export default function KingCamClone() {
  ...
  const launchProof = trpc.kingcamCloneOperatingSystem.launchFullBodyProof.useMutation({
    onSuccess: (result) => { setNotice(`KingCam's one full-body spoken-motion proof is now in the governed lane. It has one seven-second output only and a locked ${result.hardCreditCap}-credit ceiling.`); command.refetch(); },
    onError: (error) => setNotice(error.message),
  });
