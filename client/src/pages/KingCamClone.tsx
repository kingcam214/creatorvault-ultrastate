...
export default function KingCamClone() {
  ...
  const launchProof = trpc.kingcamCloneOperatingSystem.launchFullBodyProof.useMutation({
    onSuccess: (result) => { setNotice(`KingCam's full-body MiniMax proof is now in the governed motion lane. It has one five-second output only and a locked ${result.hardCreditCap}-credit ceiling.`); command.refetch(); },
    onError: (error) => setNotice(error.message),
  });
