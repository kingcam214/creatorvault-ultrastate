...
export default function KingCamClone() {
  ...
  const launchProof = trpc.kingcamCloneOperatingSystem.launchFullBodyProof.useMutation({
    onSuccess: (result) => { setNotice(`KingCam's full-body correction is now in the governed motion lane. It has one output only and a locked ${result.quote.quotedCredits}-credit ceiling.`); command.refetch(); },
    onError: (error) => setNotice(error.message),
  });
