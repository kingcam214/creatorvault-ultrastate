      ...
      <section className="relative isolate min-h-[42rem] overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto flex min-h-[42rem] max-w-6xl flex-col px-5 py-7 sm:px-8 sm:py-10 lg:px-12">
          ...
          <div className="mt-auto max-w-3xl pb-4 sm:pb-8">
            ...
            <div className="mt-7 inline-flex items-center gap-2 rounded-full border border-white/20 bg-black/35 px-3 py-2 text-[10px] font-black uppercase tracking-[.16em] text-zinc-100 backdrop-blur"><ShieldCheck className="h-3.5 w-3.5 text-[#efd18c]" /> {offers.length ? `${offers.length} playable public release${offers.length === 1 ? "" : "s"}` : "No playable public release yet"}</div>
          </div>
        </div>
      </section>

      <section className="mx-auto max-w-6xl px-5 py-14 sm:px-8 sm:py-20 lg:px-12">
        <div className="flex flex-col gap-5 border-b border-white/10 pb-7 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-[#efd18c]">Available now</p><h2 className="mt-3 text-4xl font-black leading-[.9] tracking-[-.06em] sm:text-6xl">{offers.length ? "Choose your access." : "This creator has not opened a moving public release yet."}</h2></div>{offers.length > 0 && <p className="max-w-sm text-sm leading-relaxed text-zinc-400">Each release below comes directly from this creator&apos;s active CreatorVault record and includes a playable motion preview. Prices and availability are never filled in by a template.</p>}</div>
        ...
        ) : <section className="mt-8 rounded-[2rem] border border-dashed border-white/15 bg-[#11100e] p-7 sm:p-10"><LockKeyhole className="h-7 w-7 text-[#efd18c]" /><h3 className="mt-5 text-2xl font-black">This storefront is waiting for the creator&apos;s first moving public release.</h3><p className="mt-3 max-w-xl text-sm leading-relaxed text-zinc-400">CreatorVault keeps this page honest: owner inventory stays private until the creator attaches a real playable release. This page will not fill the space with static cards, made-up prices, locked previews, subscriber counts, or purchase activity.</p></section>}
